...
train = pd.read_csv('input/train.csv', usecols=[
                    'imdb_id', 'budget', 'popularity', 'runtime', 'release_date', 'revenue'])

train = pd.merge(train, pd.read_csv(
    'input/TrainAdditionalFeatures.csv'), how='left', on=['imdb_id'])
...
train2[['release_month', 'release_day', 'release_year']] = train2['release_date'].str.split(
    '/', expand=True).replace(np.nan, 0).astype(int)
train2['release_year'] = train2['release_year']
train2.loc[(train2['release_year'] <= 18) & (
    train2['release_year'] < 100), "release_year"] += 2000
train2.loc[(train2['release_year'] > 18) & (
    train2['release_year'] < 100), "release_year"] += 1900
...
record = dict()
lgb_1 = lgb.train(params_lgb, lgb.Dataset(X_train, y_train), num_boost_round=100000, valid_sets=[lgb.Dataset(val_x, val_y)], early_stopping_rounds=500, callbacks=[lgb.record_evaluation(record)]
                  )
...
# create heat map
train2 = train2[['budget', 'rating', 'totalVotes', 'popularity',
                 'runtime', 'release_year', 'release_month', 'release_day', 'revenue']]
